...
export function ForgotPasswordPage() {
  ...
  async function submit(event) {
    ...
    try {
      ...
      setMessage(response?.message || "If an account exists, we sent password reset instructions.");
    } catch (caughtError) {
      setFormError(caughtError?.message || "We could not request a password reset");
    ...
    <AppShell>
      <AuthCard title="Forgot password">
        <form onSubmit={submit}>
          ...
          <button disabled={isSubmitting} style={{ minHeight: 44 }} type="submit">{isSubmitting ? "Sending…" : "Send reset instructions"}</button>
        </form>
